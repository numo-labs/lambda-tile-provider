...
function insertAll (id, items, callback) {
  ...
  waterfall(generateInsertItemFunctions(items, `${id}.tile`), function (_, sortKeys) {
    // Finalize the input with an index file.
    const filteredSortKeys = sortKeys.filter(key => { if (key) return key; });
    insert(`${id}.tiles`, filteredSortKeys.join(','), function (_, indexSortKey) {
...
function insert (key, value, callback) {
  ...
  dynamodb.putItem(item, function (err) {
    if (err) {
      // We will never throw an error. We will log that we failed to store an item
      // but we want to continue with the other items of course.
  ...
  return items.map((item, index) => {
    if (index) {
      return (sortKeys, next) => {
        insert(key, JSON.stringify(item), function (_, sortKey) {
